...
v1.2.1 - bug fix for depreciated numpy functions
"""
        ...
        * 'Scipy.Signal <https://docs.scipy.org/doc/scipy/reference/signal.html>'_
        ...
        if self.sampling_frequency() == 1.28:
            ...
            def datawell_tukey(n, alpha):
                '''
                Custom implementation of the Tukey window, which does not use (N-1) terms, just N
                '''
...
class displacements(object):
    ...
    def directional_spectra(self):
        ...
        for df in list_df:
            ...
            df_cospectra = df_periodogram[['hh', 'nn', 'ww', 'nw']].apply(lambda x: x.to_numpy().real)
            df_quadspectra = df_periodogram[['hn', 'hw']].apply(lambda x: x.to_numpy().imag)
